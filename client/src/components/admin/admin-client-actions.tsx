...
export function AdminClientActionsSection({
  clientId,
  accountStatus,
  suspensionReason,
  suspendedAt,
  ...
  async function handleSuspend() {
    ...
    setSubmitting(true);
    setError(null);
    const res = await apiFetch(`/api/admin/clients/${clientId}/suspend`, {
      method: "POST",
    ...
    if (!res.ok) {
      ...
      setError(j.error ?? "Could not suspend employer.");
      return;
    }
  ...
  async function handleReactivate() {
    ...
    setError(null);
    const res = await apiFetch(`/api/admin/clients/${clientId}/reactivate`, {
      method: "POST",
      credentials: "include",
    });
    ...
    if (!res.ok) {
      const j = (await res.json().catch(() => ({}))) as { error?: string };
      setError(j.error ?? "Could not reactivate employer.");
      return;
    }
    onUpdated();
  }
    ...
    <section className="rounded-2xl border border-[color:var(--border)] bg-[color:var(--surface)] p-6 shadow-sm">
      ...
      {confirmOpen ? (
        ...
        >
          ...
          >
            <h3
              id="suspend-employer-title"
              className="text-lg font-semibold text-[color:var(--text)]"
            >
              Suspend employer account?
            </h3>
            <p className="mt-2 text-sm text-[color:var(--text-muted)]">
              The employer will not be able to sign in until the account is reactivated. Enter the
